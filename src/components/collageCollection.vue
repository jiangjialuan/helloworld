<template>
  <el-dialog id="collageCollection" title="选择拼团同步" :visible.sync="dialogSelectGoodsVisible"  :top="screeHeight<=800?(screeHeight-630)/2+'px':'15vh'">
    <div class="tableList" @scroll.passive="getMore($event);">
      <table class="el-table el-table--border" cellspacing="0" cellpadding="0" border="0">
        ...
        <tr v-if="!has_next&&videoList.length>10">
          <td class="alignCenter" colspan="2">已加载全部拼团商品</td>
        </tr>
        <tr v-if="videoList.length<=0">
          <td class="alignCenter" colspan="2" >暂时没有拼团商品</td>
        </tr>
      </table>
    </div>
    <div slot="footer" class="dialog-footer">
      <el-button @click="dialogSelectGoodsVisible = false">取 消</el-button>
      <el-button type="primary" @click="selectComfirm">确 定</el-button>
    </div>
  </el-dialog>
</template>
        ...
        getList(){
          if(this.throttle(new Date().getTime())){
            if(this.has_next&&this.loading){
              this.loading=false;
              this.api.request({
                url: this.api.get_parent_groupon,
                params: {
                  buid:this.curuid,
                  page:this.page
                },
